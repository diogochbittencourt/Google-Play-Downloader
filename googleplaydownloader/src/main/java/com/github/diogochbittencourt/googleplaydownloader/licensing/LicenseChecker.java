...
import com.github.diogochbittencourt.googleplaydownloader.licensing.util.Base64;
import com.github.diogochbittencourt.googleplaydownloader.licensing.util.Base64DecoderException;
...
/**
 * Client library for Android Market license verifications. <p> The LicenseChecker is configured via a {@link Policy}
 * which contains the logic to determine whether a user should have access to the application. For example, the Policy
 * can define a threshold for allowable number of server or client failures before the library reports the user as not
 * having access. <p> Must also provide the Base64-encoded RSA public key associated with your developer account. The
...
public class LicenseChecker implements ServiceConnection {
    ...
    /**
     * Checks if the user should have access to the app.  Binds the service if necessary. <p> NOTE: This call uses a
     * trivially obfuscated string (base64-encoded).  For best security, we recommend obfuscating the string that is
     * passed into bindService using another method of your own devising. <p> source string:
     * "com.android.vending.licensing.ILicensingService" <p>
     */
    ...
    /**
     * Inform the library that the context is about to be destroyed, so that any open connections can be cleaned up. <p>
     * Failure to call this method can result in a crash under certain circumstances, such as during screen rotation if
     * an Activity requests the license check or when the user exits the application.
     */
