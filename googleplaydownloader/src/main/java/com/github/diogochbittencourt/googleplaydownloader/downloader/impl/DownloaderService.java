...
package com.github.diogochbittencourt.googleplaydownloader.downloader.impl;
...
import com.github.diogochbittencourt.googleplaydownloader.downloader.Constants;
import com.github.diogochbittencourt.googleplaydownloader.downloader.DownloadProgressInfo;
import com.github.diogochbittencourt.googleplaydownloader.downloader.DownloaderServiceMarshaller;
...
import com.github.diogochbittencourt.googleplaydownloader.downloader.IDownloaderService;
import com.github.diogochbittencourt.googleplaydownloader.downloader.IStub;
import com.github.diogochbittencourt.googleplaydownloader.licensing.AESObfuscator;
import com.github.diogochbittencourt.googleplaydownloader.licensing.APKExpansionPolicy;
import com.github.diogochbittencourt.googleplaydownloader.licensing.LicenseChecker;
import com.github.diogochbittencourt.googleplaydownloader.licensing.LicenseCheckerCallback;
import com.github.diogochbittencourt.googleplaydownloader.licensing.Policy;

import java.io.File;


/**
 * Performs the background downloads requested by applications that use the Downloads provider. This service does not
 * run as a foreground task, so Android may kill it off at will, but it will try to restart itself if it can. Note that
 * Android by default will kill off any process that has an open file handle on the shared (SD Card) partition if the
...
public abstract class DownloaderService extends CustomIntentService implements IDownloaderService {
    ...
    /**
     * The download exceeds the recommended maximum size for this network, the user must confirm for this download to
    ...
    /**
     * For intents used to notify the user that a download exceeds a size threshold, if this extra is true, WiFi is
     * required for this download size; otherwise, it is only recommended.
     */
    ...
    /**
     * This download has successfully completed. Warning: there might be other status values that indicate success in
     * the future. Use isSucccess() to capture the entire category.
     *
     * @hide
     */
    ...
    /**
     * This download has completed with an error. Warning: there will be other status values that indicate errors in the
     * future. Use isStatusError() to capture the entire category.
     */
    public static final int STATUS_UNKNOWN_ERROR = 491;

    /**
     * This download couldn't be completed because of a storage issue. Typically, that's because the filesystem is
     * missing or full. Use the more specific {@link #STATUS_INSUFFICIENT_SPACE_ERROR} and {@link
    ...
    /**
     * This download couldn't be completed because of an HTTP redirect response that the download manager couldn't
     * handle.
     *
     * @hide
     */
    ...
    /**
     * This download couldn't be completed due to insufficient storage space. Typically, this is because the SD card is
     * full.
     *
     * @hide
     */
    ...
    /**
     * This download couldn't be completed because no external storage device was found. Typically, this is because the
    ...
    /**
     * Updates the network type based upon the type and subtype returned from the connectivity manager. Subtype is only
     * used for cellular signals.
     */
    ...
    /**
     * Starts the download if necessary. This function starts a flow that does ` many things. 1) Checks to see if the
     * APK version has been checked and the metadata database updated 2) If the APK version does not match, checks the
     * new LVL status to see if a new download is required 3) If the APK version does match, then checks to see if the
     * download(s) have been completed 4) If the downloads have been completed, returns NO_DOWNLOAD_REQUIRED The idea is
     * that this can be called during the startup of an application to quickly ascertain if the application needs to
     * wait to hear about any updated APK expansion files. Note that this does mean that the application MUST be run for
     * the first time with a network connection, even if Market delivers all of the files.
     *
     * @return true if the app should wait for more guidance from the downloader, false if the app can continue
     */
    public static int startDownloadServiceIfRequired(Context context,
    ...
    /**
     * The APK has been updated and a filename has been sent down from the Market call. If the file has the same name as
     * the previous file, we do nothing as the file is guaranteed to be the same. If the file does not have the same
     * name, we download it if it hasn't already been delivered by Market.
     *
    ...
    /**
     * We use this to track network state, such as when WiFi, Cellular, etc. is enabled when downloads are paused or in
     * progress.
     */
    ...
    /**
     * This is the main thread for the Downloader. This thread is responsible for queuing up downloads and other
     * goodness.
     */
    @Override
