...
import com.github.diogochbittencourt.googleplaydownloader.downloader.DownloaderClientMarshaller;
 ...
 * It handles multiple status types; Some require user interaction and some do not. Some of the user interactions may be
 * transient. (for example: the user is queried to continue the download on 3G when it started on WiFi, but then the
 * phone locks onto WiFi again so the prompt automatically goes away) <p/> The application interface for the downloader
 * also needs to understand and handle these transient states.
 */
